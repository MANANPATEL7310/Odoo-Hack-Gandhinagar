...
export function initNotificationSubscribers() {
  // 1. Asset Allocated
  eventBus.subscribe("asset.allocated", async (payload: { assetId: string; holderEmployeeId?: string; assetTag: string }) => {
  ...
  eventBus.subscribe("transferrequest.created", async (payload: {
    requestId: string;
    assetId: string;
    assetTag: string;
  ...
  // 3. Transfer Approved
  eventBus.subscribe("transferrequest.approved", async (payload: {
    requestId: string;
    assetId: string;
    assetTag: string;
    requestedById: string;
    ...
    if (payload.targetEmployeeId && payload.targetEmployeeId !== payload.requestedById) {
      await notificationService.create(
  ...
  // 4. Transfer Rejected
  eventBus.subscribe("transferrequest.rejected", async (payload: {
    requestId: string;
    assetTag: string;
    requestedById: string;
    reason: string;
  ...
  eventBus.subscribe("maintenancerequest.resolved", async (payload: {
    ...
    if (payload.currentHolderId && payload.currentHolderId !== payload.raisedById) {
      await notificationService.create(
  ...
  // 10. Role Changed
  eventBus.subscribe("employee.role_changed", async (payload: {
    employeeId: string;
    newRole: string;
  }) => {
    await notificationService.create(
